...
{
    public enum ResourceType { Population, Gold, Food, Steel, Horse, Wood, Sword, Shield }
    ...
    public class Resources
    {
        ...
        public Resources()
        {
            ...
            _resources[ResourceType.Wood] = Balance.StartWood;
            _resources[ResourceType.Sword] = Balance.StartSword;
            _resources[ResourceType.Shield] = Balance.StartShield;
        }
    ...
    public class Army
    {
        ...
        public int Desert(int amount)
        {
            ...
            int deserted = 0;
            foreach (var unit in FullArmy.ToList())
            {
                if (amount <= 0) break;
                int loss = Math.Min(unit.Amount, amount);
                unit.TakeCasualties(loss);
                amount -= loss;
                deserted += loss;
            }
            FullArmy.RemoveAll(u => u.Amount == 0);
    ...
    public class Settlement
    {
        ...
        public (int laidOff, int deserted) EnforcePopulationCap()
        {
            int overflow = -FreePopulation();
            ...
            {
                if (overflow <= 0) break;
                int release = Math.Min(building.OccupiedSeats, overflow);
                building.ChangeSeats(-release);
                overflow -= release;
                laidOff += release;
            }

            int deserted = overflow > 0 ? Army.Desert(overflow) : 0;

            return (laidOff, deserted);
        }
        ...
        public string Summary()
        {
            return $"^^^^^^^^  Неделя: {Week} ^^^^^^^^ \nЗолото: {Resources.Get(ResourceType.Gold)}, Еда: {Resources.Get(ResourceType.Food)}, " +
                   $"Сталь: {Resources.Get(ResourceType.Steel)}, Лошадь: {Resources.Get(ResourceType.Horse)}, " +
                   $"Дерево: {Resources.Get(ResourceType.Wood)}, Меч: {Resources.Get(ResourceType.Sword)}, Щит: {Resources.Get(ResourceType.Shield)} " +
    ...
    public class Campaign
    {
        ...
        public int InvasionWeek { get; }
        public int LordDemandWeek { get; }
        public int CrusadeWeek { get; }
        public int LordTroopsRequired { get; } = Balance.LordTroopsRequired;
        public int NextCaravanWeek { get; private set; }

        public Campaign(IRandom rng)
        {
            _rng = rng;
            InvasionWeek = rng.Next(Balance.InvasionWeekMin, Balance.InvasionWeekMax + 1);
            LordDemandWeek = InvasionWeek + rng.Next(Balance.LordDemandGapMin, Balance.LordDemandGapMax + 1);
            CrusadeWeek = LordDemandWeek + rng.Next(Balance.CrusadeGapMin, Balance.CrusadeGapMax + 1);
            NextCaravanWeek = rng.Next(Balance.CaravanIntervalMin, Balance.CaravanIntervalMax + 1);
        }

        // Лорд объявил сбор и идёт обратный отсчёт до похода (для подсказки в меню деревни).
        public bool MusterAnnounced(int week) => week >= LordDemandWeek && week < CrusadeWeek;

        public IGameState? CheckStoryTrigger(int week, GameContext ctx)
        {
            // Караван: приходит раз в несколько дней и восстанавливает рыночные цены.
            if (week >= NextCaravanWeek)
            {
                ctx.TradeSystem.ResetModifiers();
                ctx.View.Show("Прибыл торговый караван — рыночные цены восстановлены.");
                NextCaravanWeek = week + _rng.Next(Balance.CaravanIntervalMin, Balance.CaravanIntervalMax + 1);
            }

            if (week == InvasionWeek)
                return new InvasionState(ctx);

            if (week == LordDemandWeek)
            {
                ctx.View.Show($"\n*** Лорд собирает войско для крестового похода! ***\n" +
                              $"Соберите минимум {LordTroopsRequired} бойцов к неделе {CrusadeWeek} — иначе поход без вас, и это поражение.");
                return null;
            }
    ...
    public static class BuildingFactory
    {
        public static bool TryCreate(string name, out Building? building)
        {
            building = name switch
            {
                ...
                "лесопилка" => new Building("Лесопилка", ResourceType.Wood, Balance.SawmillEfficiency, Balance.SawmillSeats),
                _ => null
            };
    ...
    public readonly struct WeekReport
    {
        public int Immigrants { get; init; }
        public int Starved { get; init; }
        public int LaidOffWorkers { get; init; }
        public int Deserters { get; init; }
    ...
    public class EconomySystem
    {
        ...
        public WeekReport ProcessWeek(Settlement settlement)
        {
            ...
            var (laidOff, deserted) = settlement.EnforcePopulationCap();

            settlement.NewWeek();
            return new WeekReport { Immigrants = immigrants, Starved = starved, LaidOffWorkers = laidOff, Deserters = deserted };
        }
        ...
        private int Immigrate(Settlement settlement)
        {
            ...
            int immigrants = Math.Min(surplus / Balance.ImmigrationFoodPerPerson, Balance.ImmigrationMaxPerWeek);
            if (immigrants > 0)
    ...
    {
        public Dictionary<ResourceType, double> Modifier { get; } =
            Balance.MarketPrices.Keys.ToDictionary(k => k, _ => Balance.ModifierStart);
    ...
    {
        public bool Craft(Settlement settlement, CraftingRecipe recipe, int times = 1)
        {
            if (times <= 0) return false;

            foreach (var cost in recipe.Cost)
                if (settlement.Resources.Get(cost.Key) < cost.Value * times)
                    return false;

            foreach (var cost in recipe.Cost)
                settlement.Resources.Spend(cost.Key, cost.Value * times);

            settlement.Resources.Add(recipe.Output, recipe.OutputAmount * times);
            return true;
        }

        public int MaxCraftable(Settlement settlement, CraftingRecipe recipe)
        {
            int max = int.MaxValue;
            foreach (var cost in recipe.Cost)
            {
                int available = settlement.Resources.Get(cost.Key);
                int possible = available / cost.Value;
    ...
    public abstract class BehaviorBase : IBehavior
    {
        ...
        protected readonly double _reserveHoldThreshold;

        protected BehaviorBase(IRandom rng, double reserveThreshold)
        {
            _rng = rng;
            _reserveHoldThreshold = reserveThreshold;
        }

        public void DecideReserve(Army army)
        {
            if (_rng.NextDouble() > _reserveHoldThreshold)
            {
    ...
    public class GameContext
    {
        ...
        public GameContext(Settlement settlement, BattleSystem battleSystem, EconomySystem economySystem, RecruitmentSystem recruitmentSystem,
            BuildingSystem buildingSystem, TradeSystem tradeSystem, CraftingSystem craftingSystem, Campaign campaign, IGameView view, IRandom rng)
        {
    ...
    public class RaidState : BattleStateBase
    {
        ...
        public override IGameState? Update()
        {
            ...
            return new VillageWeekState(_context);
        }
        ...
        private void ApplyReward(BattleResult result)
        {
            ...
            if (result == BattleResult.PlayerWon)
            {
                ...
                res.Add(ResourceType.Wood, Balance.RaidLootWood);
                _context.View.Show($"Добыча: золото +{gold}, сталь +{Balance.RaidLootSteel}, лошади +{Balance.RaidLootHorse}. дерево + {Balance.RaidLootWood}");
            }
    ...
    public class VillageWeekState : IGameState
    {
        readonly GameContext _context;
        public VillageWeekState(GameContext context) => _context = context;
        public IGameState? Update()
        {
            while (true)
            {
                _context.View.Show(_context.Settlement.Summary());
                if (_context.Campaign.MusterAnnounced(_context.Settlement.Week))
                    _context.View.Show($">>> Сбор в крестовый поход: нужно {_context.Campaign.LordTroopsRequired} бойцов к неделе " +
                                       $"{_context.Campaign.CrusadeWeek} (в армии сейчас {(int)_context.Settlement.Army.FullScaleArmy()}).");
                _context.View.Show("Опции: \n1 - Построить здание\n2 - Рабочие\n3 - Торговля\n4 - Мастерская\n5 - Найм отрядов\n6 - Рейд\n7 - Закончить неделю");
                int choice = _context.View.ReadChoice(1, 7);
                switch (choice)
                {
                    case 1: OpenBuildMenu(); break;
                    case 2: OpenWorkerMenu(); break;
                    case 3: OpenTradeMenu(); break;
                    case 4: OpenCraftMenu(); break;
                    case 5: OpenRecruitMenu(); break;
                    case 6: return new RaidState(_context);
                    case 7: return EndWeek();
                }
            }
        }

        private IGameState EndWeek()
        {
            WeekReport report = _context.EconomySystem.ProcessWeek(_context.Settlement);

            if (report.Starved > 0)
                _context.View.Show($"От голода погибло: {report.Starved}.");
            if (report.LaidOffWorkers > 0)
                _context.View.Show($"Не хватает людей — освобождено рабочих мест: {report.LaidOffWorkers}.");
            if (report.Deserters > 0)
                _context.View.Show($"Не хватает людей на всех — дезертировало бойцов: {report.Deserters}.");
            if (report.Immigrants > 0)
                _context.View.Show($"В поселение прибыло иммигрантов: {report.Immigrants}.");

            if (!_context.Settlement.IsAlive)
                return new GameOverState(_context);
            return _context.Campaign.CheckStoryTrigger(_context.Settlement.Week, _context) ?? new VillageWeekState(_context);
        }
        ...
        private void OpenBuildMenu()
        {
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case 4: BuildSelected("лесопилка"); break;
                    case 5: return;
                }
            }
        }
        ...
        private void OpenTradeMenu()
        {
            while (true)
            {
                var resources = Balance.MarketPrices.Keys.ToList();
                _context.View.Show(_context.Settlement.Summary());
                _context.View.Show("\nТорговать:");
                for (int i = 0; i < resources.Count; i++)
                {
                    var entry = Balance.MarketPrices[resources[i]];
                    _context.View.Show($"{i + 1} - {Balance.ResourceNames[resources[i]]} " +
                        $"(продать за {_context.TradeSystem.CurrentSellPrice(resources[i])} / купить за {entry.BasePrice})");
                }
                int back = resources.Count + 1;
                _context.View.Show($"{back} - Назад");

                int choice = _context.View.ReadChoice(1, back);
                if (choice == back) return;

                TradeResource(resources[choice - 1]);
            }
        }
        ...
        private void OpenCraftMenu()
        {
            while (true)
            {
                var recipes = Balance.Recipes;
                _context.View.Show("\nМастерская:");
                for (int i = 0; i < recipes.Length; i++)
                {
                    var r = recipes[i];
                    string cost = string.Join(", ", r.Cost.Select(kv => $"{Balance.ResourceNames[kv.Key]} {kv.Value}"));
                    _context.View.Show($"{i + 1} - {r.Name} x{r.OutputAmount} (нужно: {cost})");
                }
                int back = recipes.Length + 1;
                _context.View.Show($"{back} - Назад");
                ...
                int maxCraftable = _context.CraftingSystem.MaxCraftable(_context.Settlement, recipe);
                if (maxCraftable == 0)
                {
                    _context.View.Show("Не хватает ресурсов даже на одну единицу.");
                    continue;
                }

                _context.View.Show($"Сколько создать? (максимум {maxCraftable})");
                int times = _context.View.ReadChoice(1, maxCraftable);

                if (_context.CraftingSystem.Craft(_context.Settlement, recipe, times))
                    _context.View.Show($"Создано: {recipe.Name} x{times * recipe.OutputAmount}!");
                else
                    _context.View.Show("Не хватает ресурсов.");
            }
        }
    ...
    {
        // --- Стартовые ресурсы ---
        public const int StartPopulation = 250;
        ...
        public const int StartSword = 0;
        public const int StartShield = 0;

        // --- Экономика ---
        public const float FoodConsumptionBase = 0.5f;
        ...
        public const int ImmigrationFoodThreshold = 100; // нужен запас еды выше порога
        public const int ImmigrationFoodPerPerson = 20;  // 1 иммигрант на каждые N еды сверх порога
        public const int ImmigrationMaxPerWeek = 30;      // потолок притока за неделю

        // --- Бой ---
        public const int MaxBattleRounds = 12;
        ...
        // --- Торговля ---
        public const double SellModifierDecay = 0.8;
        ...
        public const int LightInfantryPower = 20;
        ...
        {
            { ResourceType.Food, new MarketEntry { BasePrice = 10, BatchSize = 30 } },
            { ResourceType.Steel, new MarketEntry { BasePrice = 10, BatchSize = 30 } },
            { ResourceType.Horse, new MarketEntry { BasePrice = 30, BatchSize = 10 } },
            { ResourceType.Wood, new MarketEntry { BasePrice = 10, BatchSize = 45 } },
            { ResourceType.Sword, new MarketEntry { BasePrice = 10, BatchSize = 20 } },
            { ResourceType.Shield, new MarketEntry { BasePrice = 10, BatchSize = 30 } },
        };

        public static readonly CraftingRecipe SwordRecipe = new CraftingRecipe
        {
            Name = "Меч",
            Output = ResourceType.Sword,
            OutputAmount = 1,
        ...
        public static readonly CraftingRecipe ShieldRecipe = new CraftingRecipe
        {
            Name = "Щит",
            Output = ResourceType.Shield,
            OutputAmount = 1,
            Cost = new Dictionary<ResourceType, int>
            {
                { ResourceType.Wood, 1 },
        ...
        public static readonly CraftingRecipe[] Recipes = { SwordRecipe, ShieldRecipe };
        ...
        public const int RaidLootSteel = 30;
        public const int RaidLootHorse = 10;
        public const int RaidLootWood = 40;
        ...
        // --- Сюжет (окна недель, рандом в диапазоне для реиграбельности) ---
        public const int InvasionWeekMin = 13;
        public const int InvasionWeekMax = 15;
        public const int LordDemandGapMin = 5;   // после нашествия — лорд объявляет сбор
        public const int LordDemandGapMax = 7;
        public const int CrusadeGapMin = 8;      // от объявления до похода — время собрать войско
        public const int CrusadeGapMax = 12;
        public const int LordTroopsRequired = 300; // минимум бойцов, чтобы уйти в поход (иначе поражение)
        public const int CaravanIntervalMin = 2;  
        public const int CaravanIntervalMax = 4;

        // --- Составы: нашествие / финальный враг / контингент лорда ---
        public const int InvasionCavalry = 1;
        public const int InvasionInfantry = 1;
        public const int InvasionMilitia = 2;
        ...
        public const int LordHostCavalry = 18;       // ~450
        public const int LordHostInfantry = 25;      // ~1000
        public const int LordHostMilitia = 6;        // ~300   → лорд ~1475
        public const int LordHostPerMerit = 1;       // +отряды за каждый RaidsDone

        // --- Здания (эффективность, мест) ---
        public const float ManorEfficiency = 1.25f;
        public const int ManorSeats = 90;
        public const float SmithyEfficiency = 0.5f;
        public const int SmithySeats = 50;
        public const float StableEfficiency = 0.2f;
        public const int StableSeats = 20;
        public const float SawmillEfficiency = 0.85f;
        public const int SawmillSeats = 60;
        ...
        public static readonly Dictionary<ResourceType, string> ResourceNames = new()
        {
            ...
            { ResourceType.Horse, "Лошадь" },
            { ResourceType.Wood, "Дерево" },
            { ResourceType.Sword, "Меч" },
            { ResourceType.Shield, "Щит" },
        };
    }
}